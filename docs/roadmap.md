...
**进展记录（P8A/P8B，2026-07-06）**：引入 `@earendil-works/pi-tui@0.80.3`，Node engine 提升到 `>=22.19.0`；新增可选 `--tui` 模式，保留 `-p` 与 readline REPL；基础 TUI 支持 editor 提交、assistant markdown/text 流式、tool_start/tool_update/tool_result 文本渲染、`/help`/`/compact`/扩展 command prompt action、Ctrl+C abort/退出；运行中 Agent 注入 TUI `UiCapability` facade（confirm/input/select/notify）；TUI `/help` 文本版内置命令列表、autocomplete（slash commands + 文件路径）、`/name`、`/sessions` 文本列表、`/tree` 文本列表与 `/tree <id>`、文本版 `/fork [<id>]`、文本版 `/resume <id|path|timestamp|name>`、`/follow <text>` follow-up 队列、`!`/`!!` 用户 bash、`user_bash` 扩展事件、`/compact` start/end/aborted 状态、compaction Esc abort、abort 后 pending tool 标记、基础 footer（cwd/session/model/context usage）已落地。验证：`npm run check` 全绿、`npm test` 69 测试全绿、tmux TTY 冒烟（mock provider 返回 `tui smoke ok`；`/name p8b-smoke` + `/sessions` 显示新 session；`/tree` 显示 user-message jump points）。未完成：启动期 project trust 的 TUI 化、selector 版 `/tree`/`/fork`/`/sessions`、model/thinking selector、themes、renderer/shortcut API、`/reload`。
