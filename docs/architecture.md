...
会话状态就是 `Agent.messages` 数组；传入 `session: SessionRecorder` 时每条消息在 push 点持久化为 pi v3 JSONL entry（P4 重审确认：会话是记录、内存是事实源；压缩改写内存并落 compaction entry，`messagesFromPath` 统一实现压缩感知重建，restore 与内存共用）。恢复 = `restoreSession(store)` → `initialMessages`。CLI 布局：`~/.tau/sessions/--<cwd slug>--/<时间戳>_<uuid>.jsonl`，`--continue` 原文件续写。**压缩**：`config.contextWindow` 配置后在 turn 之间自动触发（`tokens > contextWindow - reserveTokens`）；`/compact` 手动。
