...
| `agent.ts` | `Agent` 类：`prompt(input, signal)` 返回 AsyncGenerator<AgentEvent>。循环 = 发请求 → 流式收 → 执行工具 → 重复直到无工具调用。扩展钩子挂在循环各点 |
| `tools.ts` | `Tool = ToolDefinition + execute(args, signal)`，`ToolResult = { output, isError? }`，参数校验辅助函数 |
| `capabilities.ts` | 能力接口：`FileSystem`（readTextFile/writeTextFile/listDir/stat + cwd）、`Shell`（exec）。**能力可选**：宿主不提供就少注册工具 |
...
- 处理器按注册顺序链式执行，transform 串联，block/handled 短路。
- `registerTool(tool)`（同名后注册者覆盖）、`registerCommand(name, options)`。
