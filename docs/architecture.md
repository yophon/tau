...
> 最后更新：2026-07-19（P15 权限与审批：policy.ts 档位×风险矩阵、Agent 审批门、validateToolArgs、trust digest、permissions.json，D20）
...
├─ 内核层（禁止碰运行时，门禁强制）──────────────┤
│  @yophon/tau-kernel     agent 循环 · OpenAI 兼容流式    │
│                  客户端 · SSE 解析 · 工具系统 ·  │
│                  扩展系统 · 能力接口定义         │
└──────────────────────────────────────────────┘
```
...
| `compaction.ts` | **pi 压缩算法整体移植**：token 估算（usage+启发式双层；启发式为 CJK 加权 `estimateStringTokens`——CJK 1 token/字 + 其余 chars/4，偏离 pi 的纯 chars/4，D19-1）、切点选择（split-turn 感知）、摘要 prompt（pi 逐字 + 追加标识符保护条款，D19-2）、迭代更新、runCompaction 编排。摘要用同一 OpenAICompatConfig（D3） |
...
- 九个包（kernel / host-node / host-browser / host-weapp / host-rn / ext-subagents / ext-mcp / ext-resources / cli）以 `@yophon/tau-*` 锁步版本发布到 npm。
