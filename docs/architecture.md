...
会话状态就是 `Agent.messages` 数组；传入 `session: SessionRecorder` 时每条消息在 push 点持久化为 pi v3 JSONL entry（会话是记录，不是事实源——P4 compaction 时重审此方向）。恢复 = `restoreSession(store)` → `initialMessages`。CLI 布局：`~/.tau/sessions/--<cwd slug>--/<时间戳>_<uuid>.jsonl`，`--continue` 原文件续写。
