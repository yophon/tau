...
**v1 范围裁决（用户，2026-07-16）**：不做语音（后续）；仅局域网（公网路径已由 P19 的 mcp-server `--tunnel`（Cloudflare Quick Tunnel）补上，见 phase-19 规格书）；进 tau monorepo 当示例工程（不开新仓库）。**已知产品约束**：agent 循环活在手机进程，iOS 锁屏 ~30s 后冻结——v1 以"任务执行时保持亮屏"为使用前提（Android 可后续用前台服务缓解），配合会话持久化做重开恢复（v1 内存对话，见范围外）。
